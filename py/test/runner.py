# MetMuseumCollection SDK test runner
...
class MetMuseumCollectionTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MetMuseumCollectionTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MetMuseumCollectionTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MetMuseumCollectionTestRunner.getenv("MET_MUSEUM_COLLECTION_TEST_LIVE")
        override = MetMuseumCollectionTestRunner.getenv("MET_MUSEUM_COLLECTION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MetMuseumCollectionTestRunner.getenv(key)
        ...
        explain = MetMuseumCollectionTestRunner.getenv("MET_MUSEUM_COLLECTION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MetMuseumCollectionTestRunner._test_control is not None:
            return MetMuseumCollectionTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MetMuseumCollectionTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MetMuseumCollectionTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MetMuseumCollectionTestRunner.load_test_control()
...
def load_env_local():
    MetMuseumCollectionTestRunner.load_env_local()


def env_override(m):
    return MetMuseumCollectionTestRunner.env_override(m)


def entity_data(v):
    return MetMuseumCollectionTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MetMuseumCollectionTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MetMuseumCollectionTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MetMuseumCollectionTestRunner.load_test_control()


def live_delay_ms():
    return MetMuseumCollectionTestRunner.live_delay_ms()
