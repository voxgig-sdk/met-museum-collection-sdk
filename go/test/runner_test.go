...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("METMUSEUMCOLLECTION_TEST_LIVE") == "TRUE" ||
		os.Getenv("METMUSEUMCOLLECTION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("METMUSEUMCOLLECTION_TEST_EXPLAIN"); explain != "" {
