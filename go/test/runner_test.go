...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MET_MUSEUM_COLLECTION_TEST_LIVE") == "TRUE" ||
		os.Getenv("MET_MUSEUM_COLLECTION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MET_MUSEUM_COLLECTION_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.MetMuseumCollectionSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.MetMuseumCollectionSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.MetMuseumCollectionError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.MetMuseumCollectionSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.MetMuseumCollectionError{Msg: msg, Code: code}
}
