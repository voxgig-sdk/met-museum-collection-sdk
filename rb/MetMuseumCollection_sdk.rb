...
require_relative 'MetMuseumCollection_types'


class MetMuseumCollectionSDK
  ...
  def initialize(options = {})
    ...
    utility = MetMuseumCollectionUtility.new
    @_utility = utility
    ...
    feature_opts = MetMuseumCollectionHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MetMuseumCollectionHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MetMuseumCollectionFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MetMuseumCollectionUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MetMuseumCollectionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MetMuseumCollectionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MetMuseumCollectionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = MetMuseumCollectionSpec.new({
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MetMuseumCollectionError.new(
        "#{op}_allow",
        "MetMuseumCollectionSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue MetMuseumCollectionError => err
    ...
    if fetched.is_a?(Hash)
      status = MetMuseumCollectionHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MetMuseumCollectionError.new(
        "graphql_error", "MetMuseumCollectionSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MetMuseumCollectionSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
