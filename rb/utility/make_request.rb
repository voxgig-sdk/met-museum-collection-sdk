...
module MetMuseumCollectionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MetMuseumCollectionResponse.new({})
    ...
    if fetch_err
      ...
      response = MetMuseumCollectionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MetMuseumCollectionResponse.new(fetched)
    else
