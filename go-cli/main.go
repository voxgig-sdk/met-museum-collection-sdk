// Command MetMuseumCollection-cli is a CLI + REPL for the MetMuseumCollection SDK,
...
const prompt = "met-museum-collection"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "department object search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MET_MUSEUM_COLLECTION_APIKEY (api key), MET_MUSEUM_COLLECTION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MET_MUSEUM_COLLECTION_APIKEY carries the API key and
	// MET_MUSEUM_COLLECTION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MET_MUSEUM_COLLECTION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MET_MUSEUM_COLLECTION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMetMuseumCollectionSDK(opts)
