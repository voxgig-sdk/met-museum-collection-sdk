# met-museum-collection-cli

boru-driven command-line client **and** interactive REPL for the MetMuseumCollection
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/met-museum-collection-cli)
make build

# 2. See usage (words, entities, env vars)
./met-museum-collection-cli --help

# 3. Provide credentials once, via the environment
export MET_MUSEUM_COLLECTION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./met-museum-collection-cli list department
./met-museum-collection-cli list object

# 5. Override the API base URL for a single call
MET_MUSEUM_COLLECTION_BASE=https://api.example.com ./met-museum-collection-cli list department

# 6. No arguments -> interactive REPL
./met-museum-collection-cli
met-museum-collection> list department
met-museum-collection> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/met-museum-collection-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/met-museum-collection-cli list department
   ```
...
Run with no arguments to open a REPL (prompt `met-museum-collection>`). Each line is
evaluated as its own boru expression:

```text
$ ./met-museum-collection-cli
met-museum-collection> list department
met-museum-collection> /help
met-museum-collection> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `department`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/met-museum-collection-cli`. |
...
(`./met-museum-collection-cli <expr>`) and interactively (the REPL), and expressions compose the
