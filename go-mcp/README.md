# met-museum-collection-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MetMuseumCollection SDK as
two agent tools — `met-museum-collection_list` and `met-museum-collection_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/met-museum-collection-mcp)
make build

# 2. Provide credentials via the environment
export MET_MUSEUM_COLLECTION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user met-museum-collection \
  -- /absolute/path/to/met-museum-collection-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./met-museum-collection-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// met-museum-collection_list: first page of records
{ "entity": "department" }
{ "entity": "department", "query": { } }

// met-museum-collection_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/met-museum-collection-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user met-museum-collection \
     -- "$PWD"/dist/*/met-museum-collection-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `met-museum-collection_list` and `met-museum-collection_load` tools now appear
   in new sessions. Ask the agent to *"list department using met-museum-collection"*
   and it calls `met-museum-collection_list` with `{"entity":"department"}`.
...
```sh
export MET_MUSEUM_COLLECTION_APIKEY=sk_live_xxx            # API key
export MET_MUSEUM_COLLECTION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `met-museum-collection_list` tool
...
### Call the `met-museum-collection_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `met-museum-collection_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `met-museum-collection_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MET_MUSEUM_COLLECTION_APIKEY` | API key sent with every request. |
| `MET_MUSEUM_COLLECTION_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

department | object | search

### Smoke test via HTTP (raw JSON-RPC)

```sh
./met-museum-collection-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"met-museum-collection_load","arguments":{"entity":"object","query":{"id":1}}}}'
```
