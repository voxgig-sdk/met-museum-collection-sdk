...
// MetMuseumCollection SDK feature test
//
...
class FtCtrl extends MetMuseumCollectionControl
{
...
class FtClient
{
    ...
    private MetMuseumCollectionUtility $utility;

    public function __construct(MetMuseumCollectionUtility $utility, array $options)
    {
    ...
    public function get_utility(): MetMuseumCollectionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MetMuseumCollectionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new MetMuseumCollectionUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (MetMuseumCollectionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MetMuseumCollectionConfig::make_feature($name);
    ...
    private static function buildUrl(MetMuseumCollectionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MetMuseumCollectionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MetMuseumCollectionResponse($fetched);
        }
    ...
    private function fail(MetMuseumCollectionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MetMuseumCollectionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
