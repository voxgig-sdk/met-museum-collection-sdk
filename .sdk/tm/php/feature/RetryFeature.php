...
// MetMuseumCollection SDK retry feature
...
class MetMuseumCollectionRetryFeature extends MetMuseumCollectionBaseFeature
{
    ...
    public function init(MetMuseumCollectionContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MetMuseumCollectionContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MetMuseumCollectionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MetMuseumCollectionContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
