...
The Lua SDK for the MetMuseumCollection API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Department(nil):list(nil, nil)
...
local result, err = client:MetMuseumCollection(nil):load(
  { id = "test01" }, nil
)
