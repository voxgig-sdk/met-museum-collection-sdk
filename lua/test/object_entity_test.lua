...
describe("ObjectEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MET_MUSEUM_COLLECTION_TEST_OBJECT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local object_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.object")))
    local object_ref01_data = nil
    if #object_ref01_data_raw > 0 then
      object_ref01_data = helpers.to_map(object_ref01_data_raw[1][2])
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(object_ref01_data_dt0_loaded)
...
function object_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MET_MUSEUM_COLLECTION_TEST_OBJECT_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["MET_MUSEUM_COLLECTION_TEST_OBJECT_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["MET_MUSEUM_COLLECTION_TEST_LIVE"] == "TRUE" then
  ...
  return {
    ...
    env = env,
    explain = env["MET_MUSEUM_COLLECTION_TEST_EXPLAIN"] == "TRUE",
    live = live,
