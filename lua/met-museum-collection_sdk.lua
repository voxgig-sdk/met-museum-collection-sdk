...
require("met-museum-collection_types")
...
local MetMuseumCollectionSDK = {}
MetMuseumCollectionSDK.__index = MetMuseumCollectionSDK
...
MetMuseumCollectionSDK._make_feature = _make_feature


function MetMuseumCollectionSDK.new(options)
  local self = setmetatable({}, MetMuseumCollectionSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MetMuseumCollectionSDK:options_map()
...
function MetMuseumCollectionSDK:get_utility()
  return Utility.copy(self._utility)
end


function MetMuseumCollectionSDK:get_root_ctx()
  return self._rootctx
end


function MetMuseumCollectionSDK:prepare(fetchargs)
...
function MetMuseumCollectionSDK:direct(fetchargs)
...
function MetMuseumCollectionSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function MetMuseumCollectionSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "MetMuseumCollectionSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function MetMuseumCollectionSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MetMuseumCollectionSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MetMuseumCollectionSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function MetMuseumCollectionSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MetMuseumCollectionSDK.new(sdkopts)
  sdk.mode = "test"
