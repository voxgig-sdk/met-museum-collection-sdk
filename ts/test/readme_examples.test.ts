...
//     (\`new MetMuseumCollectionSDK(...)\` / \`MetMuseumCollectionSDK.test(...)\` become
//     \`MetMuseumCollectionSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MetMuseumCollectionSDK'
...
const TEST_SEED = {"entity":{"department":{"test01":{"id":"test01"}},"object":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MetMuseumCollectionSDK(...)\` and any
// \`MetMuseumCollectionSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MetMuseumCollectionSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MetMuseumCollectionSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
