...
// MetMuseumCollection SDK Object entity
...
class ObjectEntity
{
    ...
    {
        if ($args) {
            $this->_data = MetMuseumCollectionHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = MetMuseumCollectionHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws MetMuseumCollectionError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MetMuseumCollectionHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
